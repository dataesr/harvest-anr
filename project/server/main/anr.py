...
def harvest_anr_projects(project_type, cache_participant):
    ...
    if project_type=='ANR':
        ...
        for col in ['code_projet_anr', 'instrument_financement', 'intitule_du_comite']:
            assert(col in df_aapg)
        for k in df_aapg.to_dict(orient='records'):
            #current_id = k['Code projet ANR']
            current_id = k['code_projet_anr']
    ...
    for e in df_projects.to_dict(orient='records'):
        ...
        if code_decision in enrichment_dict:
            if 'instrument_financement' in enrichment_dict[code_decision]:
                new_elt['instrument'] = enrichment_dict[code_decision]['instrument_financement']
            if 'intitule_du_comite' in enrichment_dict[code_decision]:
                new_elt['comite'] = enrichment_dict[code_decision]['intitule_du_comite']
